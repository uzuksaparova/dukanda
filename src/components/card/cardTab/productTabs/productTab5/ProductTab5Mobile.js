...
import './productTab5Mobile.scss';

function ProductTab5Mobile(props) {
    const {
        TabContainer,
        setProductImages,
        setIsProductImageModalOpen,
    ...
    const imagesSection = (type, header, typeImages) => {
        return (
            <div className="mobile-sections">
                <span className="mobile-image-header"> {header} </span>
                <div className="mobile-image-all">
                    ...
                    {typeImages.map((image, index) => {
                        return (
                            <div className="mobile-image-hover" key={image.id}>
                                {type === 'normal' && image?.main ? (
                                    <BsStarFill className="mobile-main-star" />
                                ) : null}
                                <img
                                    alt={image.image}
                                ...
                                />
                                <div className="mobile-edit-hover">
                                    {authorization[decodedToken.role].includes(
                                        'uploadProductImage'
                                    ) ? (
                                        <>
                                            <div className="mobile-edit-icon-hover">
                                                <input
                                                    className="mobile-image-replace-input"
                                                    key={image.id}
                                                    type="file"
                                                    accept=".jpg,.jpeg,.png"
                                                    onChange={(e) =>
                                            ...
                                            {type === 'normal' ? (
                                                <div
                                                    className="mobile-edit-icon-hover"
                                                    onClick={() =>
                                                        onMakeMainClick(
                                                            image.id
                                                        )
                                                ...
                                                >
                                                    <BsStarFill className="mobile-edit-icon" />
                                                </div>
                                            ) : null}
                                            <div
                                                className="mobile-edit-icon-hover"
                                                onClick={() =>
                                                    deleteImage(type, image.id)
                                                }
                                            >
                                                <BsTrashFill className="mobile-edit-icon" />
                                            </div>
                                        </>
                                    ) : null}
                                    <div
                                        className="mobile-preview-icon-hover"
                                        onClick={() =>
                                            handleImagePreview(
                                                typeImages,
                                                index
                                    ...
                                    >
                                        <MdZoomOutMap className="mobile-edit-icon" />
                                    </div>
                                </div>
                            </div>
                        );
    ...
    return (
        <TabContainer>
            <div className="mobile-product-tab5-images">
...
export default connect(mapStateToProps, mapDispatchToProps)(ProductTab5Mobile);
